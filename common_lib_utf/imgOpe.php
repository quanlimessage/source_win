...
function save( $dstRoot="", $dstImageName="", $width="", $height="", $aspect="" ){
	...
	// リサンプル＆リサイズして保存
		...
		if( $this->isFixed ){		// アスペクト比固定（元画像に合わせる）
				...
				$aspect_ratio2 = $this->dstW / $this->dstH;
...
//					if ( abs( $this->srcW - $this->dstW ) < abs( $this->srcH - $this->dstH ) ){
					if ( $aspect_ratio < $aspect_ratio2 ){
						// アップ画像のアスペクト比が、設定サイズのアスペクト比よりも小さい場合
						// $this->dstH はそのまま
						$this->dstW = ceil( $aspect_ratio * $this->dstH );	// 横サイズ
					}else{
						// アップ画像のアスペクト比が、設定サイズのアスペクト比よりも大きい場合
						// $this->dstW はそのまま
		...
		for ( $i = 0; $i < $this->rootNum; $i++ ){
			...
			switch( $this->fileType ):
				case IMAGETYPE_GIF:  $up_result = ImageJPEG( $outputImg, $outName ); break;
				case IMAGETYPE_JPEG: $up_result = ImageJPEG( $outputImg, $outName ); break;
				case IMAGETYPE_PNG:  $up_result = ImagePNG( $outputImg, $outName ); break;
			endswitch;
			if ( !$up_result ) return false;
