...
function path_maker(){
	...
	$demo_flg = 0;// デモページフラグ(本番なら0,デモなら1)
	...
	if(sslchk()){//sslページの場合
		...
		if($demo_flg==1){
			...
			$tmp_url = 'www.'.$domain_name;
		}

		//全て絶対パスにする
		$path_data[0]	 = "http://".$tmp_url.$dest_path;
	...
	}elseif(strpos($_SERVER['PHP_SELF'],"zeeksdg")!=""){//zeeksdgのデモページの場合　SSLが一切無い設定(zeeksdg)
