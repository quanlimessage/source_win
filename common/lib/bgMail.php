...
class bgMail{
	...
	//var $program_path = "/home/demo0.zeek.jp/www/common_lib/sendmailCLI.php";	// sendmailCLI.phpのパス
	var $program_path = "/home/zeek/common_lib/sendmailCLI.php";	// sendmailCLI.phpのパス
...
function sendmail( $em, $sub="", $body="", $name="", $adr="" ){
	...
	$data_file = getcwd() . '/mail_tmp' . date("YmdHis") . '.ini';
	$fp = fopen( $data_file, "w" ) or exit("ファイルオープンエラー");
	flock( $fp, LOCK_EX );
...
// 
// 		○使用例
// 			$bgMail = new bgMailDB(SUBJECT, BODY, NAME, ADDRESS);
