...
public static function strRep($str,$mode){
	...
	switch($mode):
		...
		case 2:		//改行コードを除去するモード
			...
			if(!empty($str)){
				if(is_array($str)):
					for($i=0;$i<count($str);$i++)$str[$i] = preg_replace("/[\r\n]/","",$str[$i]);
				else:
					$str = preg_replace("/[\r\n]/","",$str);
				endif;
			}
...
public static function strCheck($str,$mode,$mes){
	...
	switch($mode):
		...
		case 1:		# 空白文字を含んでいればエラー
			if(preg_match("/[[:blank:]]|[[:space:]]/",$str)){$errmes .= "{$mes}<br>\n";}
			break;
		case 2:		# 数字以外の文字が使われていたらエラー
			if(preg_match("/[^0-9]/",$str)){$errmes .= "{$mes}<br>\n";}
			break;
		case 3:		# 半角英数字以外の文字が使われていたらエラー
			if(preg_match("/[^_a-zA-Z0-9]/",$str)){$errmes .= "{$mes}<br>\n";}
			break;
