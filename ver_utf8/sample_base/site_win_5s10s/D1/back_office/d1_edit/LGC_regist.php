...
// 新規登録
	...
	if($fetchCNT[0]["CNT"] < D1_DBMAX_CNT):
		...
		if($_POST["regist_type"]=="new" && $ins_chk == 1){
			$vosql ="UPDATE D1_OUTPUT SET VIEW_ORDER = VIEW_ORDER+1";
			// ＳＱＬを実行
			if(!empty($vosql)){
			...
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM D1_OUTPUT WHERE(DISPLAY_FLG = '1')";
			$fetchVO = dbOpe::fetch($vosql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
			$view_order = ($fetchVO[0]["VO"] + 1);
		}

		$sql = "
		INSERT INTO D1_OUTPUT(
