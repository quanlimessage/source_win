	...
	for($i=0;$i < count($select_stock);$i++){
		...
		// 対象記事IDデータのチェック
			...
			//削除対象はRES_IDが一致するファイル
				search_file_del(S1_IMG_PATH,$select_stock[$i]."*");

			//データベースから削除
				//登録データの削除
				$db_result = dbOpe::regist("DELETE FROM ".S1_PRODUCT_LST." WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
...
// 一括表示の切替（フラグを更新）
	...
	for($i=0;$i < count($select_stock);$i++){
		...
		// 対象記事IDデータのチェック
			...
			//表示するに切り替える
				$db_result = dbOpe::regist("UPDATE ".S1_PRODUCT_LST." SET DISPLAY_FLG = '1' WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
...
// 一括非表示の切替（フラグを更新）
	...
	for($i=0;$i < count($select_stock);$i++){
		...
		// 対象記事IDデータのチェック
			...
			//非表示するに切り替える
				$db_result = dbOpe::regist("UPDATE ".S1_PRODUCT_LST." SET DISPLAY_FLG = '0' WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
