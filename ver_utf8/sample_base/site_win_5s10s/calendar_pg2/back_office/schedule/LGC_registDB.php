...
#===============================================================================
# 休日ボタンが押されたら送信データをチェックし、DBの管理者情報を更新する
#===============================================================================
switch($_POST["action"]):
	...
	case "insert":
		...
		$db_result = dbOpe::regist($sql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
		if($db_result){die("データの更新に失敗しました<hr>{$db_result}");}else{$msg="休日を更新しました。";}
	...
	case "delete":
		...
		$db_result = dbOpe::regist($sql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
		if($db_result){die("データの更新に失敗しました<hr>{$db_result}");}else{$msg="休日を削除しました。";}

endswitch;

//

function GetActionMode($action_mode , $d ) {
	$sql = "
		SELECT
