...
// 一括データの完全削除
	...
	for($i=0;$i < count($select_stock);$i++){
		...
		// 対象記事IDデータのチェック
			...
			//データベースから削除
				//登録データの削除
				$db_result = dbOpe::regist("DELETE FROM ".S7_3_PRODUCT_LST." WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");

			//並び順の削除
				$db_result = dbOpe::regist("DELETE FROM ".S7_3_VIEW_ORDER_LIST." WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");

			//並び順補佐の削除
				$db_result = dbOpe::regist("DELETE FROM ".S7_3_VIEW_ORDER_LIST2." WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
		}
...
// 一括表示の切替（フラグを更新）
	...
	for($i=0;$i < count($select_stock);$i++){
		...
		// 対象記事IDデータのチェック
			...
			//表示するに切り替える
				$db_result = dbOpe::regist("UPDATE ".S7_3_PRODUCT_LST." SET DISPLAY_FLG = '1' WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
...
// 一括非表示の切替（フラグを更新）
	...
	for($i=0;$i < count($select_stock);$i++){
		...
		// 対象記事IDデータのチェック
			...
			//非表示するに切り替える
				$db_result = dbOpe::regist("UPDATE ".S7_3_PRODUCT_LST." SET DISPLAY_FLG = '0' WHERE(RES_ID = '".$select_stock[$i]."')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
