...
#-----------------------------------------------------------------
		...
		if($_POST["copy_type"]=="new"){
			//複製する時のVIEW_ORDER
			$vosql_old = "SELECT VIEW_ORDER AS VO FROM ".S3A_PRODUCT_LST." WHERE (RES_ID = '$res_id') AND (DEL_FLG = '0')";
			$fetchVO_old = dbOpe::fetch($vosql_old,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
			$view_order_old = $fetchVO_old[0]["VO"];

			$vosql ="UPDATE ".S3A_PRODUCT_LST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE (VIEW_ORDER > $view_order_old)";
			if(!empty($vosql)){
			...
			//トップ登録チェックを入れる時
			$vosql ="UPDATE ".S3A_PRODUCT_LST." SET VIEW_ORDER = VIEW_ORDER+1";
			if(!empty($vosql)){
				$db_result = dbOpe::regist($vosql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
			}
			...
			//新規登録
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".S3A_PRODUCT_LST." WHERE(DEL_FLG = '0')";
...
// 対象IDのデータ更新
	...
	if($_POST["regist_type"]=="update" && $del_img){
		foreach($del_img as $k => $v){
		 	search_file_del(S3A_IMG_PATH,$res_id."_".$v.".*");
		}
	}
...
// 新規登録
	...
	$cnt_sql = "SELECT COUNT(*) AS CNT FROM ".S3A_PRODUCT_LST." WHERE(DEL_FLG = '0')";
	$fetchCNT = dbOpe::fetch($cnt_sql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);

	if($fetchCNT[0]["CNT"] < S3A_DBMAX_CNT):

		$sql = "
		INSERT INTO ".S3A_PRODUCT_LST."
			SET
				RES_ID = '$res_id',
...
$imgObj = new imgOpe(S3A_IMG_PATH);
...
if(is_uploaded_file($_FILES['up_img']['tmp_name'][$i])){
	...
	search_file_del(S3A_IMG_PATH,$for_imgname."_".$i.".*");
