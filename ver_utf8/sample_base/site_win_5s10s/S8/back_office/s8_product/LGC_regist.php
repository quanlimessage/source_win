...
#-----------------------------------------------------------------
		...
		if($_POST["copy_type"]=="new"){
			$vosql_old = "SELECT VIEW_ORDER AS VO FROM ".S8_PRODUCT_LST." WHERE (RES_ID = '$res_id') AND (DEL_FLG = '0')";
			$fetchVO_old = $PDO -> fetch($vosql_old);
			$view_order_old = $fetchVO_old[0]["VO"];

			$vosql ="UPDATE ".S8_PRODUCT_LST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE (CATEGORY_CODE = '$ca') AND (VIEW_ORDER > $view_order_old)";
			$PDO -> regist($vosql);
			$view_order = ($fetchVO_old[0]["VO"] + 1);
		}elseif($_POST["ins_chk"]=="1"){
		//トップ登録チェックを入れる時
			$vosql ="UPDATE ".S8_PRODUCT_LST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE (CATEGORY_CODE = '$ca')";
			$PDO -> regist($vosql);
			$view_order = 1;
		}
		else{
		//新規登録
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".S8_PRODUCT_LST." WHERE (CATEGORY_CODE = '$ca') AND (DEL_FLG = '0')";
...
// 新規登録
	...
	$cnt_sql = "SELECT COUNT(*) AS CNT FROM ".S8_PRODUCT_LST." INNER JOIN ".S8_CATEGORY_MST." ON (".S8_PRODUCT_LST.".CATEGORY_CODE = ".S8_CATEGORY_MST.".CATEGORY_CODE) WHERE(".S8_PRODUCT_LST.".DEL_FLG = '0')AND(".S8_CATEGORY_MST.".DEL_FLG = '0')";
