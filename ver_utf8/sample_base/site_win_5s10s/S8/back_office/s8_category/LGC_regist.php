...
// 新規登録
	...
	//カテゴリーは無制限で登録が可能
		...
		if($_POST["regist_type"]=="new" && $ins_chk == 1){
			$vosql ="UPDATE ".S8_CATEGORY_MST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE(DEL_FLG = '0') AND (PARENT_CATEGORY_CODE = '$pca')";
			if(!empty($vosql)){
			...
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".S8_CATEGORY_MST." WHERE(DEL_FLG = '0') AND (PARENT_CATEGORY_CODE = '$pca')";
			$fetchVO = dbOpe::fetch($vosql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
			$view_order = ($fetchVO[0]["VO"] + 1);
		}

		$sql = "
		INSERT INTO ".S8_CATEGORY_MST."(
