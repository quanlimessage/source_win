    ...
    // 新規登録
        ...
        //カテゴリーは無制限で登録が可能
            ...
            if ($_POST["regist_type"]=="new" && $ins_chk == 1) {
                $vosql ="UPDATE ".COLUMN_CATEGORY_MST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE(DEL_FLG = '0')";
                $PDO -> regist($vosql);
                $view_order = 1;
            } else {
                $vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".COLUMN_CATEGORY_MST." WHERE(DEL_FLG = '0')";
                $fetchVO = $PDO -> fetch($vosql);
                $view_order = ($fetchVO[0]["VO"] + 1);
            }

            $sql = "
            INSERT INTO ".COLUMN_CATEGORY_MST."(
                CATEGORY_NAME,
                VIEW_ORDER,
                DEL_FLG,
                DISPLAY_FLG
