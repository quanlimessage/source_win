...
// 該当データの完全削除
	...
	// SQL実行
	$db_result = dbOpe::regist("DELETE FROM ".W1_LINK." WHERE(RES_ID = '$res_id')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
...
// 表示/非表示の切替（フラグを更新）
	...
	// SQLを実行
	$db_result = dbOpe::regist("UPDATE ".W1_LINK." SET DISPLAY_FLG = '$up_display' WHERE(RES_ID = '$res_id')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
	if($db_result)die("DB登録失敗しました<hr>{$db_result}");
