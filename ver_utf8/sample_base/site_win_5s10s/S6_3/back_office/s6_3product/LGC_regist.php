...
#-----------------------------------------------------------------
		...
		if($_POST["copy_type"]=="new"){
			//複製する時のVIEW_ORDER
			$vosql_old = "SELECT VIEW_ORDER AS VO FROM ".S6_3PRODUCT_LST." WHERE (RES_ID = '$res_id') AND (DEL_FLG = '0')";
			$fetchVO_old = $PDO -> fetch($vosql_old);
			$view_order_old = $fetchVO_old[0]["VO"];

			$vosql ="UPDATE ".S6_3PRODUCT_LST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE (VIEW_ORDER > $view_order_old)";
			$PDO -> regist($vosql);
			$view_order = ($fetchVO_old[0]["VO"] + 1);
		}elseif($ins_chk == 1){
			//トップ登録チェックを入れる時
			$vosql ="UPDATE ".S6_3PRODUCT_LST." SET VIEW_ORDER = VIEW_ORDER+1";
			$PDO -> regist($vosql);
			$view_order = 1;
		}
		else{
			//新規登録
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".S6_3PRODUCT_LST." WHERE(DEL_FLG = '0')";
...
// 対象IDのデータ更新
	...
	if($_POST["regist_type"]=="update" && $del_img){
		foreach($del_img as $k => $v){
		 	search_file_del(S6_3IMG_PATH,$res_id."_".$v.".*");
		}
	}
...
// 新規登録
	...
	$cnt_sql = "SELECT COUNT(*) AS CNT FROM ".S6_3PRODUCT_LST." WHERE(DEL_FLG = '0')";
	$fetchCNT = $PDO -> fetch($cnt_sql);

	if($fetchCNT[0]["CNT"] < S6_3DBMAX_CNT):

		$sql = "
		INSERT INTO ".S6_3PRODUCT_LST."
			SET
			RES_ID = '$res_id',
...
$imgObj = new imgOpe(S6_3IMG_PATH);
...
if(is_uploaded_file($_FILES['up_img']['tmp_name'][$i])){
	...
	search_file_del(S6_3IMG_PATH,$for_imgname."_".$i.".*");
	...
	// 画像のアップロード：画像名は(記事ID_画像番号.jpg)
		...
		//$imgObj->isFixed=true;//自動算出処理（画像サイズ固定の場合コメントアウト）
