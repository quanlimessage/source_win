...
#=================================================================================
	...
	if($del_pdf){//チェックがされているか判定
		...
		// FILE削除
		if(file_exists(IMG_PATH.$filename.".".$old_extension)){
			unlink(IMG_PATH.$filename.".".$old_extension) or die("ファイルの削除に失敗しました。");
		}
	...
	/////////////////////////////////////////////////////
	//ファイルの登録処理
	if((W3_DBMAX_CNT > $fetch_PDF_NUM[0]['CNT']) || (file_exists(IMG_PATH.$res_id.".".$old_extension))){//ファイルのアップ制限が超えてないかチェックを行う、また古いファイルを更新する場合も許可する
		...
		if(is_uploaded_file($_FILES['up_file_pdf']['tmp_name'])):
			...
			if($_FILES['up_file_pdf']['size'] < (LIMIT_SIZE * 1024 *1024)){
				...
				//古いファイルを削除する（ファイルタイプが変更された場合の対応）
					if(file_exists(IMG_PATH.$res_id.".".$old_extension) ){
						unlink(IMG_PATH.$res_id.".".$old_extension) or die("ファイルの削除に失敗しました。");
					}

				//ファイルのアップロード処理
					if(!copy($_FILES['up_file_pdf']['tmp_name'],IMG_PATH.$filename.".".$extension)){
						exit("アップロード処理に失敗しました。");
					}else{
						//本番アップ時に権限が変わりパーミッションの変更ができない場合、
						//処理は失敗したままで次の処理へ行かせる。
						@chmod(IMG_PATH.$filename.".".$extension, 0666);
					}
