...
// 新規登録
	...
	//カテゴリーは無制限で登録が可能
		...
		if($_POST["regist_type"]=="new" && $ins_chk == 1){
			$vosql ="UPDATE ".CP1_CATEGORY_MST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE(DEL_FLG = '0')";
			if(!empty($vosql)){
			...
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".CP1_CATEGORY_MST." WHERE(DEL_FLG = '0')";
