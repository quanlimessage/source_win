...
#-----------------------------------------------------------------
		...
		if($_POST["copy_type"]=="new"){
			$vosql_old = "SELECT VIEW_ORDER AS VO FROM ".CP1_PAGE_LST." WHERE (RES_ID = '$res_id') AND (DEL_FLG = '0')";
			$fetchVO_old = dbOpe::fetch($vosql_old,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
			$view_order_old = $fetchVO_old[0]["VO"];
				
			$vosql ="UPDATE ".CP1_PAGE_LST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE (CATEGORY_CODE = '$ca') AND (VIEW_ORDER > $view_order_old)";
			if(!empty($vosql)){
		...
		//トップ登録チェックを入れる時
			$vosql ="UPDATE ".CP1_PAGE_LST." SET VIEW_ORDER = VIEW_ORDER+1 WHERE (CATEGORY_CODE = '$ca')";
			if(!empty($vosql)){
				$db_result = dbOpe::regist($vosql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);
				if($db_result)die("DB登録失敗しました<hr>{$db_result}");
			}
		...
		//新規登録
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM ".CP1_PAGE_LST." WHERE (CATEGORY_CODE = '$ca') AND (DEL_FLG = '0')";
...
// 対象IDのデータ更新
	...
	if($_POST["regist_type"]=="update" && $del_img){
		foreach($del_img as $k => $v){
		 	search_file_del(CP1_IMG_PATH,$res_id."_".$v.".*");
		}
	}
...
// 新規登録
	...
	$cnt_sql = "SELECT COUNT(*) AS CNT FROM ".CP1_PAGE_LST." INNER JOIN ".CP1_CATEGORY_MST." ON (".CP1_PAGE_LST.".CATEGORY_CODE = ".CP1_CATEGORY_MST.".CATEGORY_CODE) WHERE(".CP1_PAGE_LST.".DEL_FLG = '0')AND(".CP1_CATEGORY_MST.".DEL_FLG = '0')";
	$fetchCNT = dbOpe::fetch($cnt_sql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);

	//最大登録件数に達していない、そして、カテゴリーが存在している場合登録をする
	if(($fetchCNT[0]["CNT"] < CP1_DBMAX_CNT) && count($fetchCA)):
		
		$sql = "
		INSERT INTO ".CP1_PAGE_LST."
			SET
			RES_ID = '$res_id',
...
if(is_uploaded_file($_FILES['up_img']['tmp_name'][$i])){
	...
	search_file_del(CP1_IMG_PATH,$for_imgname."_".$i.".*");
