...
// 対象IDのデータ更新
	...
	if($_POST["regist_type"]=="update" && $del_img == 1){
		...
		if(file_exists(N4_1IMG_PATH.$res_id.".jpg")){
			unlink(N4_1IMG_PATH.$res_id.".jpg") or die("画像の削除に失敗しました。");
		}
...
// 画像ファイル名の決定（新しいIDを生成して使用。DB登録時のRES_IDにも使用）
	...
	$cnt_sql = "SELECT COUNT(*) AS CNT FROM N4_1TICKER WHERE(DEL_FLG = '0')";
	$fetch = dbOpe::fetch($cnt_sql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);

	if($fetch[0]["CNT"] < N4_1DBMAX_CNT){
		$sql = "
		INSERT INTO N4_1TICKER(
			RES_ID,COMMENT,CONTENT,DISP_DATE,DISPLAY_FLG
		)
		VALUES(
			'$res_id','$comment','$content','$disp_time','$display_flg'
		)";
	}
	else{
		header("Location: ./");
...
$imgObj = new imgOpe(N4_1IMG_PATH);
