...
#---------------------------------------------------------------------
    ...
    //	$fetch = $dbh->fetch($sql);
            ...
            if ($_SESSION['LOGIN'] !== true && !$err_msg) {
...
if ($login || $_GET['login']) {
    $result = login($login_id, $login_pass);
}

if ($logout || $_GET['logout'] == 1) {
    logout();
}
...
/*
$sql = "SELECT BO_ID AS user , BO_PW AS password FROM BLOG_APP_INIT_DATA";
