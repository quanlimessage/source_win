...
// 該当データの完全削除
	...
	//カテゴリーのDELフラグを設定
	$db_result = dbOpe::regist("UPDATE ".S7_CATEGORY_MST." SET DEL_FLG = '1' WHERE(CATEGORY_CODE = '$cate')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
	...
	$db_result = dbOpe::regist("UPDATE ".S7_PRODUCT_LST." SET DEL_FLG = '1' WHERE(CATEGORY_CODE = '$cate')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
...
// 表示/非表示の切替（フラグを更新）
	...
	// SQLを実行
	$db_result = dbOpe::regist("UPDATE ".S7_CATEGORY_MST." SET DISPLAY_FLG = '$up_display' WHERE(CATEGORY_CODE = '$cate')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
