...
// 対象IDのデータ更新
	...
	if($_POST["regist_type"]=="update" && $del_img){
		foreach($del_img as $k => $v){
		 	search_file_del(N1_IMG_PATH,$res_id."_".$v.".*");
		}
	}
...
// 新規登録
	...
	$cnt_sql = "SELECT COUNT(*) AS CNT FROM ".N1_WHATSNEW." WHERE(DEL_FLG = '0')";
	$fetch = dbOpe::fetch($cnt_sql,DB_USER,DB_PASS,DB_NAME,DB_SERVER);

	if($fetch[0]["CNT"] < N1_DBMAX_CNT){

		$sql = "
		INSERT INTO ".N1_WHATSNEW."
			SET
				RES_ID = '$res_id',
				$sql_update_data
		";
...
if(is_uploaded_file($_FILES['up_img']['tmp_name'][$i])){
	...
	search_file_del(N1_IMG_PATH,$for_imgname."_".$i.".*");
