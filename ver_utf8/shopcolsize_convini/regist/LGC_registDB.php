...
if($_SESSION['cust']['CUSTOMER_ID']):	// 利用経験者（UPDATE）
	...
	$lastEmailChk = dbOpe::fetch("SELECT EMAIL FROM ".CUSTOMER_LST." WHERE(EMAIL = '".$_SESSION['cust']['EMAIL']."')AND(DEL_FLG = '0') AND (EXISTING_CUSTOMER_FLG = '1')",DB_USER,DB_PASS,DB_NAME,DB_SERVER);
	if ( $lastEmailChk ){
		$_SESSION['cust'] = array();
		deleteItems();
		header("Location: ./");
...
for ( $i = 0; $i < count($cart_list); $i++ ){
	...
	$purchaseData[$i]['disc_id']			= $disc_id;							// 商品詳細(カラー/サイズ)ID
	//$purchaseData[$i]['color_id']			= $color_id;						// カラーID
	//$purchaseData[$i]['size_id']			= $size_id;							// サイズID
	$purchaseData[$i]['part_no']			= $part_no;							// 商品番号
	$purchaseData[$i]['product_name']		= $product_name;					// 商品名
	$purchaseData[$i]['color_name']			= $color_name;						// カラー名
	$purchaseData[$i]['size_name']			= $size_name;						// サイズ名
...
//////////////////////////////////////////////////
// 在庫数チェック
// もし、案件がショッピングライトの場合、在庫は無いのでここはコメントアウトさせる
//
for ( $i = 0; $i < count($purchaseData); $i++ ){
	...
	if($CntRst[0]["STOCK_QUANTITY"] < $purchaseData[$i]['quantity']){
		...
		$error_message .= "サイズ：".$purchaseData[$i]['size_name']."<br>\n";
		$error_message .= "カラー：".$purchaseData[$i]['color_name']."<br>\n";
...
for ( $i = 0; $i < count($purchaseData); $i++ ){
	...
	#-----------------------------------------------
	# 在庫数更新（クレジットとコンビニ決済の場合は行わない）
	#-----------------------------------------------

		if(($_SESSION['cust']['PAYMENT_METHOD'] != 1) && ($_SESSION['cust']['PAYMENT_METHOD'] != 4)){
		...
		if($zaiko < 1){
			$zaikogire_lst .= "商品番号：".$purchaseData[$i]["part_no"]." ".$purchaseData[$i]["product_name"];
			$zaikogire_lst .= ($purchaseData[$i]['color_name'])?" ".$purchaseData[$i]["color_name"]:"";
			$zaikogire_lst .= ($purchaseData[$i]['size_name'])?" ".$purchaseData[$i]["size_name"]:"";
			$zaikogire_lst .= "\n";
		}
		# 購入後の在庫数をDBに上書き
		$zaiko_sql ="
	...
	SET
		ORDER_ID = '$order_id',
		PROPERTY_ID = '".$purchaseData[$i]['disc_id']."',
		PART_NO = '".$purchaseData[$i]['part_no']."',
		PRODUCT_NAME = '".$purchaseData[$i]['product_name']."',
		COLOR_NAME = '".$purchaseData[$i]['color_name']."',
		SIZE_NAME = '".$purchaseData[$i]['size_name']."',
