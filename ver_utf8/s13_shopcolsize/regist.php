...
if($result == 'ok' || $result == 'OK'):
	...
	/////////////////////////////////////////////////////////////////////////////////////////////
	// 保存データが無い場合	※管理者に決済完了後にエラーがあった旨を通知後、スクリプト終了
	//
	/*
	if(!$chkCust){
		// 現在時刻
		$time_str = date("Y年n月j日H時i分s秒");

		// エラー通知メール本文
		$err_mailbody = "
		WEBサイトショッピングカートにてクレジット決済終了後にエラーが発生しました。
		至急弊社(オールインターネット)担当までご連絡ください。
	...
	// エラー通知メール送信（失敗時：強制終了）
	$webmstmail_result = mb_send_mail(WEBMST_SHOP_MAIL,$subject,$err_mailbody,$headers);
	if(!$webmstmail_result)die("Send Mail Error! for WebMaster");

	die("データ保存エラー：<a href='mailto:".WEBMST_SHOP_MAIL."'>".WEBMST_SHOP_MAIL."</a>までご連絡ください。");
	...
	*/
	...
	for ( $i = 0; $i < count($fetchPerItem); $i++ ){
		...
		if($CntRst[0]["STOCK_QUANTITY"] < $fetchPerItem[$i]['QUANTITY']):
			...
			$color_name = ($fetchPerItem[$i]["COLOR_NAME"])?$fetchPerItem[$i]["COLOR_NAME"]:"-";
			$zaiko_err_str .= "商品カラー：".$color_name."\n";
			$size_name = ($fetchPerItem[$i]["SIZE_NAME"])?$fetchPerItem[$i]["SIZE_NAME"]:"-";
			$zaiko_err_str .= "商品サイズ：".$size_name."\n";
		...
		if($zaiko < 1){
			$zaikogire_lst .= "商品番号：".$fetchPerItem[$i]["PART_NO"]." ".$fetchPerItem[$i]["PRODUCT_NAME"];
			$zaikogire_lst .= ($fetchPerItem[$i]['COLOR_NAME'])?" ".$fetchPerItem[$i]["COLOR_NAME"]:"";
			$zaikogire_lst .= ($fetchPerItem[$i]['SIZE_NAME'])?" ".$fetchPerItem[$i]["SIZE_NAME"]:"";
			$zaikogire_lst .= "\n";
		}
	...
	for($i = 0; $i < count($fetchPerItem); $i++){
		...
		if(empty($fetchPerItem[$i]['COLOR_NAME'])){
			$items .= "カラー：-\n";
		}else{
			$items .= "カラー：".$fetchPerItem[$i]['COLOR_NAME']."\n";
		}
		if(empty($fetchPerItem[$i]['SIZE_NAME'])){
			$items .= "サイズ：-\n";
		}else{
			$items .= "サイズ：".$fetchPerItem[$i]['SIZE_NAME']."\n";
		}
	...
	if(strstr($mailbody,"<ADDRESS>"))$mailbody = str_replace("<ADDRESS>",$shipping_list[$fetchOrderCust[0]["STATE"]]['pref'].$fetchOrderCust[0]["ADDRESS1"].$fetchOrderCust[0]["ADDRESS2"],$mailbody);
	if(strstr($mailbody,"<USRTEL>"))$mailbody = str_replace("<USRTEL>",$fetchOrderCust[0]["TEL1"]."-".$fetchOrderCust[0]["TEL2"]."-".$fetchOrderCust[0]["TEL3"],$mailbody);
	if(strstr($mailbody,"<ALPWD>"))$mailbody = str_replace("<ALPWD>",$alpwd,$mailbody);
	...
	// 送料がある場合は表示
	if($customer_data['shipping_amount']){
		if(strstr($mailbody,"<SHIPPING_DISP>"))$mailbody = str_replace("<SHIPPING_DISP>","　※送料（\\".number_format($fetchOrderCust[0]['shipping_amount'])."）を含みます",$mailbody);
	}
	else{
		if(strstr($mailbody,"<SHIPPING_DISP>"))$mailbody = str_replace("<SHIPPING_DISP>","",$mailbody);
	}
