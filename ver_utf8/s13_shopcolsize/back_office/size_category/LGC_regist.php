...
// 新規登録
	...
	//カテゴリーは無制限で登録が可能
		...
		if($_POST["regist_type"]=="new" && $ins_chk == 1){
			$vosql ="UPDATE SIZE_MST SET VIEW_ORDER = VIEW_ORDER+1 WHERE(DEL_FLG = '0') AND (COLOR_CODE = '$ca')";
			$PDO -> regist($vosql);
			$view_order = 1;
		}
		else{
			$vosql = "SELECT MAX(VIEW_ORDER) AS VO FROM SIZE_MST WHERE(DEL_FLG = '0') AND (COLOR_CODE = '$ca')";
